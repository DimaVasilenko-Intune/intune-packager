...
async function loadSettings() {
  const data = await chrome.storage.sync.get([
    'backendUrl', 'aiProvider', 'analyzeMode',
  ]);
  ...
  if (localData.aiKey) document.getElementById('api-key').value = localData.aiKey;
...
async function saveSettings() {
  ...
  await chrome.storage.sync.set({ backendUrl, aiProvider, analyzeMode });
  // Store API key separately in local storage (not synced)
